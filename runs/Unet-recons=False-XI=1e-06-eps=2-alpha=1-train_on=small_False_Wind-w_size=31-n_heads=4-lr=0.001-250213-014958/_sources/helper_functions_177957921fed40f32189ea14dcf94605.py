...
#         ul_set = MAESTRO(groups=train_groups, sequence_length=sequence_length, device=device)
    ...
    elif dataset == 'IDMT-Guitar':
        l_set = IDMTGuitarDataset(groups=['dataset1'],
                                sequence_length=sequence_length, device=device, refresh=refresh)
        
        ul_set = IDMTGuitarDataset(groups=['dataset2'],
                                sequence_length=sequence_length, device=device, refresh=refresh)

        validation_dataset = IDMTGuitarDataset(groups=['dataset3'],
                                            sequence_length=validation_length, device=device, refresh=refresh)
        
        full_validation = IDMTGuitarDataset(groups=['dataset3'],
                                            sequence_length=None, device=device, refresh=refresh)
    else:
        raise Exception("Please choose the correct dataset")
    # l_set → training-labeled
    # ul_set → training-unlabeled
    # validation_dataset & full_validation → for evaluation
    return l_set, ul_set, validation_dataset, full_validation

#Shahzad
def prepare_VAT_dataset2(sequence_length, validation_length, refresh, device, small=False, supersmall=False, dataset='CustomDataset'):
    if dataset == 'CustomDataset':
        # Load the custom dataset
        l_set = CustomDataset(sequence_length=sequence_length, device=device, refresh=refresh)
        ul_set = CustomDataset(sequence_length=sequence_length, device=device, refresh=refresh)
        validation_dataset = CustomDataset(sequence_length=validation_length, device=device, refresh=refresh)
        full_validation = CustomDataset(sequence_length=None, device=device, refresh=refresh)
    else:
        raise ValueError("Unsupported dataset. Use 'CustomDataset'.")
...
def train_VAT_model(model, iteration, ep,total_ep, l_loader, ul_loader, optimizer, scheduler, clip_gradient_norm, alpha, VAT=False, VAT_start=0):
    model.train()
...
#     loss = losses['loss/train_frame'] + alpha*(losses['loss/train_LDS_l']+losses['loss/train_LDS_ul'])/2
        ...
        if clip_gradient_norm:
            clip_grad_norm_(model.parameters(), clip_gradient_norm)
        print(f'Train Epoch: {ep}  out of {total_ep} [{i*batch_size}/{iteration*batch_size}'
                f'({100. * i / iteration:.0f}%)]'
                f"\tMain Loss: {sum(losses.values()):.6f}\t"
#                 + f"".join([f"{k.split('/')[-1]}={v.item():.3e}\t" for k,v in losses.items()])
                , end='\r') 
    print(' '*100, end = '\r')          
    print(f'Train Epoch: {ep} out of {total_ep}\tLoss: {total_loss/iteration:.6f}')
    return predictions, losses, optimizer

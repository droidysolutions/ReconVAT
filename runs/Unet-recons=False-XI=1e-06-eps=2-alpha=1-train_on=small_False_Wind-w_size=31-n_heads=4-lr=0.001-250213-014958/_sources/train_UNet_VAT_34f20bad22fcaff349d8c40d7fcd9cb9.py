...
def train(spec, resume_iteration, train_on, batch_size, sequence_length,w_size, n_heads, small, train_batch_size,
    ...
    supervised_set, unsupervised_set, validation_dataset, full_validation = prepare_VAT_dataset(
                                                                          ...
                                                                          supersmall=True,
                                                                          dataset='IDMT-Guitar'
                                                                         # train_on = 'CustomDataset'  # Change this to 'CustomDataset'
                                                                        #   dataset=train_on
                                                                          )  
    
    # supervised_set, unsupervised_set, validation_dataset, full_validation = prepare_VAT_dataset2(
    #                                                                       sequence_length=sequence_length,
    #                                                                       validation_length=sequence_length,
    #                                                                       refresh=refresh,
    #                                                                       device=device,
...
#     unsupervised_set = ConcatDataset([unsupervised_set, MAPS_unsupervised_set])
    print(f"    #######  {supervised_set.data} #########")
    print(f"supervised_set size: {len(supervised_set)} .")
    print(f"unsupervised_set size: {len(unsupervised_set)} .")
    print(f"validation_dataset size: {len(validation_dataset)} .")
    print(f"full_validation size: {len(full_validation)} .")
    print(f"    #######  Defuult Returning from train() #########")
...
#     scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=base_lr, max_lr=max_lr, step_size_up=step_size_up,cycle_momentum=False)
    ...
    for ep in range(1, epoches+1):
        if VAT==True:
            predictions, losses, optimizer = train_VAT_model(model, iteration, ep,epoches, supervised_loader, unsupervised_loader,
                                                             optimizer, scheduler, clip_gradient_norm, alpha, VAT, VAT_start)
        else:
            predictions, losses, optimizer = train_VAT_model(model, iteration, ep,epoches, supervised_loader, None,
                                                             optimizer, scheduler, clip_gradient_norm, alpha, VAT, VAT_start)            

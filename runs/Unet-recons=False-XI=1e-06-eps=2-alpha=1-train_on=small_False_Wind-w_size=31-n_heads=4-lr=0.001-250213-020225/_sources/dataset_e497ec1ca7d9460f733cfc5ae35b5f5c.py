...
class PianoRollAudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if audio_length < self.sequence_length:
            print(f"⚠️ Warning: Audio file {data['path']} is too short ({audio_length} samples). Padding or skipping!")
            padded_audio = torch.zeros(self.sequence_length, dtype=data['audio'].dtype)
            padded_audio[:audio_length] = data['audio']  # Copy available audio
            data['audio'] = padded_audio
            audio_length = self.sequence_length  # Set new length

        # Compute sequence slicing
